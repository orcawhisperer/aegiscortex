...
export default function Home() {
  ...
  const [activeId, setActiveId] = useState("rag_verified_fastpath");
  ...
  const modeLabel = useMemo(() => {
    if (!evalRes) return hosted ? "Simulator" : "Simulator";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setThresholds(state.thresholds || thresholds);
        ...
        if (first) {
          ...
          const raw = JSON.stringify(first.context, null, 2);
          setContextJSON(raw);
          await evaluate(first.id, raw);
        }
